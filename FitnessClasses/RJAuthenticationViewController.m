            ...
            [[Digits sharedInstance] authenticateWithViewController:nil configuration:configuration completion:^(DGTSession *session, NSError *error) {
                if (session) {
                    ...
                    [RJParseUser logInWithUsernameInBackground:session.phoneNumber
                     ...
                     {
                         if (user) {
                             [RJParseUtils fetchCurrentUserWithCompletion:^(RJParseUser *refetchedUser, BOOL fetchSuccess) {
                                 if (refetchedUser) {
...
- (void)viewDidLoad {
    ...
    NSString *title = [NSString stringWithFormat:@"Login to %@", [[RJParseTemplate currentTemplate] name]];
